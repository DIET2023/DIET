...
from .src.data import val_data_loader, write_tsv, get_tsv
...
parser = argparse.ArgumentParser(description="Evaluate models: Fine-tuning with 100% labels on real image data")
...
def main():
    ...
    # model = resnet_models.__dict__[args.arch](output_dim=args.num)   查看模型结构时使用
    # print('model is \n{}'.format(model))
    # print('model.named_parameters() is \n{}'.format(model.named_parameters()))
    # for name, param in model.named_parameters():
    #     print(name)
    # print(param)

    # build data
    val_data_path = os.path.join(args.data_path, args.val_path)
    val_dataset = val_data_loader(val_data_path)
    ...
    model = load_model(model_path=model_path, logger=logger)
    # model to gpu
    model = model.cuda()
    ...
    label_list, pred_list, image_list = validate_network(val_loader, model, val_dataset)
    print(f'label_list \n{label_list[:5]}')
    print(f'pred_list \n{pred_list[:5]}')
    print(f'image_list \n{image_list[:5]}')
    model_path: str
    ...
    tsv_path = os.path.join(args.dump_path, "prediction_{}.tsv".format(model_name))
...
def validate_network(val_loader, model, val_dataset):
    ...
    with torch.no_grad():
        ...
        for i, (inp, target) in enumerate(val_loader):
            ...
            image_path, image_target = val_dataset.samples[i]  # 注意batch size为1时才可以这样写的
            ...
            for pred in output:
                pred_list.append({"predictions": pred.cuda().data.cpu().numpy().tolist()})
